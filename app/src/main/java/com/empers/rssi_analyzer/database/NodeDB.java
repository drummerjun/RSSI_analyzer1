...
public class NodeDB extends SQLiteOpenHelper {
    ...
    private static final String TABLE_NODES = "nodes_table";    //table name
    //column names
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_MP3_TABLE = "CREATE TABLE " + TABLE_NODES + "("
    ...
    public long addNode(Node node) {
        ...
        new_id = db.insert(TABLE_NODES, null, values);
        db.close();
        return new_id;
    }
    ...
    public Node getNode(int node_id) {
        ...
        Node node = new Node();
        Cursor c = db.query(TABLE_NODES, new String[] {
                        KEY_ID, // 0
                        KEY_NAME, // 1
                        KEY_NODE_ID, // 2
                        KEY_GROUP_ID // 3
    ...
    public void updateNode(Node node) {
        ...
        values.put(KEY_NODE_ID, node.getNodeId());
        values.put(KEY_GROUP_ID, node.getGroupId());
        db.update(TABLE_NODES, values, "_id=" + node.getID(), null);
        db.close();
    }
    ...
    public void deleteNodes(int group_id) {
        SQLiteDatabase db = getWritableDatabase();
        db.delete(TABLE_NODES, KEY_GROUP_ID + " = ?", new String[] { String.valueOf(group_id) });
        db.close();
    }

    public void deleteNode(int _id) {
        SQLiteDatabase db = getWritableDatabase();
        db.delete(TABLE_NODES, KEY_ID + " = ?", new String[] { String.valueOf(_id) });
        db.close();
    }
}
